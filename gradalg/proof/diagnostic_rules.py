"""
Built-in diagnostic rules for :func:`gradalg.proof.diagnostics.diagnose`.
...
(done from :mod:`gradalg.proof.__init__`) is enough to make the rules
...
from gradalg.core.expr import Expr, Integer, Neg, Product, Rational, Sum
...
def _classifies(
    ...
    Mirrors the check in :func:`gradalg.algorithms.sort_product._degree_of`:
    scalars, numeric literals, :class:`Derivation` self-describers, and
    registry-declared :class:`Graded` factors all classify.
    """
    ...
    if registry.has(factor, Scalar):
        return True
    return registry.get(factor, Graded) is not None
