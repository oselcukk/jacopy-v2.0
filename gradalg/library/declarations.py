...
:class:`~gradalg.core.expr.Symbol`s and declare each one's grading on a
:class:`~gradalg.core.registry.PropertyRegistry`. That is two lines of
...
construction with the :class:`~gradalg.core.properties.Graded`
...
def Functions(
    ...
    *,
    ...
    SN-degree ``-1``; the
    :class:`~gradalg.brackets.schouten.ScoutenNijenhuis` expansion
...
def VectorFields(
    ...
    grading convention used by :mod:`gradalg.brackets.lie` — this is
...
def Bivector(
    ...
    :mod:`gradalg.library.poisson`: a 2-vector carries
    :class:`Graded(degree=1)` (SN-degree ``k - 1`` for a k-vector).
