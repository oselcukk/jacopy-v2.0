...
:mod:`gradalg.algorithms.sort_product`, which needs grading information.
...
def _canon_product(children: Tuple[Expr, ...]) -> Expr:
    ...
    :mod:`gradalg.algorithms.sort_product`.
    """
...
# --------------------------------------------------------------------- #
#
# These helpers live here (not in ``gradalg.core.equality``) because
...
def semantically_equal(a: Expr, b: Expr) -> bool:
    ...
    scope. Use :func:`gradalg.algorithms.sort_product` downstream when
    those are needed.
    """
