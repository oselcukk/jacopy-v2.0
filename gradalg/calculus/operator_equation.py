...
:func:`~gradalg.proof.verifier.prove_operator_equation` at every call
site.

The class does not introduce a new tactic. It reuses
:class:`~gradalg.proof.strategies.AgreementOnGenerators` as the default
...
from gradalg.proof.strategies import Strategy
from gradalg.proof.verifier import prove_operator_equation
...
class OperatorEquation:
    ...
    ----------
    lhs
        Left operator (typically an :class:`~gradalg.algebra.derivation.Derivation`,
        a :class:`~gradalg.core.expr.Sum` of derivations, or a composition).
    rhs
        Right operator, of the same degree as ``lhs``.
    algebra
        Any object exposing a ``generators`` property — the canonical
        example is :class:`~gradalg.calculus.exterior_algebra.ExteriorAlgebra`.
    """
    ...
    def prove(
        ...
        *,
        ...
        Delegates to
        :func:`~gradalg.proof.verifier.prove_operator_equation`, which
        defaults to :class:`~gradalg.proof.strategies.AgreementOnGenerators`
        over the algebra's generators. Raises
        :class:`~gradalg.proof.strategies.ProofFailure` when degrees
