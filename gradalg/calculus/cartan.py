...
standard :class:`~gradalg.proof.strategies.AgreementOnGenerators`
...
:class:`~gradalg.proof.strategies.UnrollToFoundations`) so the
...
from gradalg.proof.expansion import ExpansionEngine, default_engine
...
class CartanCalculus:
    ...
    d
        ...
        :data:`gradalg.calculus.exterior_d.d`.
    lie_derivative
        Factory ``X → L_X``.
    interior
        Factory ``X → ι_X``.
