...
:class:`~gradalg.proof.expansion.Definition` so it can be registered
with an :class:`~gradalg.proof.expansion.ExpansionEngine` and fire
...
the :class:`~gradalg.proof.expansion.DSquaredZeroDefinition`.
...
def invariant_d_one_form(
    ...
    * ``Act(ι_{[X, Y]}, ω)`` — the pairing of ``ω`` with the bracket,
    ...
    :class:`~gradalg.brackets.lie.LieBracket` singleton. ``interior``
    defaults to :func:`gradalg.calculus.interior.interior` so the
...
class InvariantDOneFormDefinition(Definition):
    ...
    :class:`~gradalg.proof.step.ProofStep`:
